...
6. Закиньте туда (`Launcher\gameFiles\libraries`) [Парсер библиотек](https://github.com/kostya-main/parser-libraries/releases) и запустите его
7. Откройте появившийся .json файл его содержимое нужно скопировать в настройку профиля `libraries`, перед этим необходимо выполнить очистку блока `libraries`, удалив оттуда все элементы с `"type": "library"`, остальные типы (`"type": "native"`) необходимо оставить
8. Выполните команду syncall и о чудо, все заработало 
